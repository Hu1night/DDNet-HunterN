		...
		else
		{
			...
			if(m_VoteUpdate)
			{
				...
				for(int i = 0; i < MAX_CLIENTS; i++)
				{
					...
					if((IsKickVote() || IsSpecVote()) && ((!g_Config.m_SvSpecVote && m_apPlayers[i]->GetTeam() == TEAM_SPECTATORS) ||
	...
	if(Server()->ClientIngame(ClientID))
	{
		...
		else if(MsgID == NETMSGTYPE_CL_CALLVOTE)
		{
			...
			if(str_comp_nocase(pMsg->m_Type, "option") == 0)
			{
				...
				else
				{
					...
					if(str_startswith(pMsg->m_Value, "☐"))
					{
						...
						if(!Instance.m_Init)
						{
							SendChatTarget(ClientID, "房间未初始化"); // language
							return;
						}
					...
					while(pOption)
					{
						if(str_comp_nocase(pMsg->m_Value, pOption->m_aDescription) == 0)
						{
							...
							{
								SendChatTarget(ClientID, "非法的选项"); // language
								return;
							}
			...
			else if(str_comp_nocase(pMsg->m_Type, "spectate") == 0)
			{
				...
				str_format(aDesc, sizeof(aDesc), "移动'%s'到观察者", Server()->ClientName(SpectateID));
		...
		else if(MsgID == NETMSGTYPE_CL_SETTEAM && !pPlayerWorld->m_Paused)
		{
			...
			{
				if(pPlayer->IsPaused())
					SendChatTarget(ClientID, "Use /pause first then you can kill"); // language
				else
				{
			...
			{
				SendChatTarget(ClientID, "You can't change your team due to balancing"); // language
			}
		}
...
void CGameContext::ConForceVote(IConsole::IResult *pResult, void *pUserData)
{
	...
	if(str_comp_nocase(pType, "option") == 0)
	{
		...
		if(!pOption)
		{
			str_format(aBuf, sizeof(aBuf), "'%s' isn't an option on this server", pValue); // language
			pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "server", aBuf);
			return;
		}
	}
	else if(str_comp_nocase(pType, "kick") == 0)
	{
		...
		{
			pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "server", "Invalid client id to kick"); // language
			return;
		}
...
int CGameContext::ProcessSpamProtection(int ClientID)
{
	...
	{
		char aBuf[128];
		str_format(aBuf, sizeof aBuf, "等待 %d 秒后你才能说话", Muted); // language
...
void CGameContext::Whisper(int ClientID, char *pStr)
{
	...
	*pStr = 0;
	...
	{
		str_format(aBuf, sizeof(aBuf), "未找到名为'%s'的玩家", pName); // language
		SendChatTarget(ClientID, aBuf);
		return;
	}
...
bool CGameContext::RateLimitPlayerVote(int ClientID)
{
	...
	if(m_VoteCloseTime)
	{
		SendChatTarget(ClientID, "等待当前投票结束才能发起新的投票"); // language
		return true;
	}
